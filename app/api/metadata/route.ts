...
export async function GET(req: NextRequest) {
  ...
  if (type === "team" && name) {
    metadata = generateMetadata({
      title: `${name} - USC Days 2025`,
      description: `Follow ${name} team's performance in USC Days 2025 competitions. View scores, schedules, and achievements.`,
      url: `/teams/${encodeURIComponent(name)}`,
      image: `/og-team-${encodeURIComponent(name.toLowerCase())}.png`,
    });
  } else if (type === "sport" && sport) {
    metadata = generateMetadata({
      title: `${sport} - USC Days 2025`,
      description: `${sport} competition results and schedules for USC Days 2025. Follow all matches and team standings.`,
