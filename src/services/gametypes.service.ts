...
import { AddGameTypePayload, DeleteGameTypePayload, EditGameTypePayload } from "@/types/gametypes.types";
...
class GameTypeService {
    ...
    async editGameType({ id, gameName }: EditGameTypePayload) {
            try {
                ...
                throw new Error('An unexpected error occurred while updating the gametype.');
            }
    }
    ...
    async deleteGameType({ id }: DeleteGameTypePayload) {
            try {
                ...
                throw new Error('An unexpected error occurred while deleting the gametype.');
            }
    }
}
